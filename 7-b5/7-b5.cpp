...
int stu_list::print(const char* prompt)
{
	...
	cout << setfill(' ') << setw(5) << "序号" << setw(9) << "学号" << setw(32) << "姓名" << setw(7) << "第一轮" << "第二轮" << endl;
	cout << setfill('=') << setw(60) << "" << endl;
	for (int i = 0; i < list_merge_num; i++)
	{
		cout << setfill(' ') << setw(5) << i + 1 << setw(9) << list_merge[rank[i]].stu_no << setw(32) << list_merge[rank[i]].stu_name << setw(7) << (list_merge[rank[i]].first_round ? 'Y' : '/') << (list_merge[rank[i]].second_round ? (list_merge[rank[i]].first_round ? "Y" : "补选"):"退课") << endl;
	}
