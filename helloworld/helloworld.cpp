...
//
//	if (1) {
//		struct tj_time* tp;/* 2051891 黄治东 信管 */
#define _CRT_SECURE_NO_WARNINGS		//使用了VS认为unsafe的函数
#include <iostream>
#include <iomanip>
#include <cstdio>
...
struct tj_time* tj_time_convert(int input_time)
{
	...
	temp -= day * 60 * 60 * 24;
	result.tj_hour = temp / (60 * 60);
	temp -= result.tj_hour * 60 * 60;
	result.tj_minute = temp / 60;
	temp -= result.tj_minute * 60;
	result.tj_second = temp;
	int year = 1970;
	bool leap;
	while (1)
	{
	...
	int month[12] = { 31,28 + leap,31,30,31,30,31,31,30,31,30,31 };
	for (int* p = month; p - month < 12; p++)
	{
		if (day >= *p)
		{
			day -= *p;
